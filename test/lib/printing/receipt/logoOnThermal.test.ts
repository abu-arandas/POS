...
import { buildReceiptDoc } from '../../../../src/lib/printing/receiptDoc';
...
describe('loadReceiptLogo', () => {
  ...
  it('resolves the decoded image', async () => {
    stubImage('load');
    const { loadReceiptLogo } = await import('../../../../src/lib/printing/receiptCanvas');
  ...
  it('resolves null when the decode never reports back at all', async () => {
    ...
    const { loadReceiptLogo, LOGO_DECODE_TIMEOUT_MS } =
      await import('../../../../src/lib/printing/receiptCanvas');
...
describe('raster path selection', () => {
  ...
  beforeEach(() => {
    ...
    vi.doMock('../../../../src/lib/printing/receiptCanvas', () => ({
      renderReceiptRaster,
      loadReceiptLogo,
  ...
  afterEach(() => {
    vi.doUnmock('../../../../src/lib/printing/receiptCanvas');
    vi.resetModules();
  });
  ...
  async function print(store: StoreSettings, layout: ReceiptLayout) {
    ...
    const { printReceipt } = await import('../../../../src/lib/printing/hardwarePrint');
