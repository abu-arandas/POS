...
interface AuthState {
  ...
  // `id` is optional so callers can pre-generate it — the PIN hash is salted
  // with the account id, which must therefore exist before hashing.
  handleAddUser: (
    name: string,
...
// Pre-hashed default PINs for seed data (Admin 1234, Manager 5555, Cashier
// 0000), salted with the account id: SHA-256("<id>:<PIN>") — see lib/hash.ts.
// Existing installs may still hold legacy unsalted hashes; those are upgraded
