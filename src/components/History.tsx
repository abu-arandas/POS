  ...
  const handlePrintReceipt = (tx: SaleTransaction) => {
    if (printerConfig.type === 'system') {
      ...
      if (printWindow) {
        ...
        alert('Standard Print Blocked: Please allow popups for direct hardware system receipts formatting!');
      }
    } else {
      alert(`ESC/POS receipt stream dispatched to active ${printerConfig.type.toUpperCase()} printer module. Cutting paper roll...`);
    }
  };

  const getPaymentIcon = (method: SaleTransaction['paymentMethod']) => {
    switch (method) {
      case 'card': return <CreditCard size={13} className="text-blue-500" />;
      case 'cash': return <DollarSign size={13} className="text-emerald-500" />;
      case 'mobile': return <Smartphone size={13} className="text-purple-500" />;
      case 'gift': return <Gift size={13} className="text-amber-500" />;
    }
  };

  return (
    <div id="history-root" className="flex-1 flex h-screen overflow-hidden bg-slate-50 p-6">
      
      {/* LEFT COLUMN: Transaction List (2/3 width) */}
      <div id="transaction-list-section" className="flex-1 flex flex-col min-w-0 pr-6 overflow-hidden">
        ...
        <div id="history-header" className="mb-6 shrink-0">
          ...
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">Audit past orders, process refunds, and view detailed printable receipts.</p>
        </div>

        {/* Filters */}
        <div id="history-filters" className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs space-y-4 mb-6 shrink-0">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all shrink-0 ${
                    dateFilter === opt.id ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                >
                  {opt.label}
                </button>
        ...
        {/* Transactions Table */}
        <div id="history-table-container" className="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                    ...
                    return (
                      <tr 
                        ...
                        className={`hover:bg-slate-50/50 transition-colors cursor-pointer ${
                          isSelected ? 'bg-slate-100/80 hover:bg-slate-100/80' : isRefunded ? 'opacity-70' : ''
                        }`}
                      >
                        ...
                        <td className="py-3 px-4 font-semibold text-slate-700 truncate">
                          {tx.customerName || <span className="text-slate-400 font-normal">Walk-In</span>}
                        </td>
          ...
          <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50 text-[10px] text-slate-400 font-mono flex justify-between shrink-0">
            ...
            <span>TOTAL VALUE: {settings.currency}{filteredTransactions.reduce((sum, t) => sum + (t.status === 'completed' ? t.total : 0), 0).toFixed(2)}</span>
          </div>
        </div>
      ...
      <div id="receipt-view-section" className="w-80 border border-slate-200 rounded-3xl bg-white shadow-xl flex flex-col overflow-hidden shrink-0">
        
        {activeTransaction ? (
          <>
            {/* Header Status */}
            <div className={`p-4 border-b flex items-center justify-between ${
              activeTransaction.status === 'refunded' ? 'bg-rose-50 border-rose-100 text-rose-800' : 'bg-emerald-50 border-emerald-100 text-emerald-800'
            }`}>
              <div className="flex items-center space-x-2">
                <Check size={16} className={activeTransaction.status === 'refunded' ? 'text-rose-600' : 'text-emerald-600'} />
                <span className="font-sans font-bold text-xs">
                  {activeTransaction.status === 'refunded' ? 'Transaction Refunded' : 'Transaction Completed'}
                </span>
              </div>
              <button 
            ...
            <div className="flex-1 p-5 overflow-y-auto bg-slate-100 flex flex-col justify-between">
              ...
              <div id="audit-receipt-mockup" className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-4 font-mono text-xs text-slate-700">
                <div className="text-center border-b border-dashed border-slate-200 pb-3">
                  <h4 className="font-bold text-slate-900 text-[11px] uppercase tracking-tight">{settings.storeName}</h4>
                ...
                <div className="space-y-1 pb-3 border-b border-dashed border-slate-200">
                  {activeTransaction.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-[11px]">
                      <span className="truncate max-w-[130px]">{item.quantity}x {item.productName}</span>
                      <span>{settings.currency}{item.total.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-1 text-[10px]">
                  ...
                  {activeTransaction.paymentMethod === 'cash' && (
                    <>
                      <div className="flex justify-between">
                        <span>CASH PAID:</span>
                        <span>{settings.currency}{(activeTransaction.cashPaid || 0).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-slate-950 font-bold">
                        <span>CASH CHANGE:</span>
                        <span>{settings.currency}{(activeTransaction.cashChange || 0).toFixed(2)}</span>
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="text-center pt-3 border-t border-dashed border-slate-200 space-y-1.5">
                  ...
                  <span className="text-[9px] text-slate-400 font-mono">AUTH: {activeTransaction.id}</span>
                </div>
              </div>
                ...
                {activeTransaction.status === 'completed' ? (
                  ...
                  <div className="bg-rose-50 border border-rose-100 p-3 rounded-xl flex items-start gap-2">
                    ...
                    <div className="text-[10px] text-rose-800 leading-normal">
                      ...
                      <p className="mt-0.5">This transaction has been refunded. Items have been added back to catalog levels, and transactions cannot be double refunded.</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400">
            ...
            <p className="text-xs text-slate-400 max-w-[200px] mt-1">Select a transaction row from the logs to view its detailed receipt & action controls.</p>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="text-center space-y-2 mb-6">
                ...
                <h3 className="font-sans font-extrabold text-base text-white">Manager Override Required</h3>
                <p className="text-xs text-slate-400">Enter a Manager or Admin passcode PIN to authorize this sales refund.</p>
              </div>
              ...
              <form onSubmit={handleAuthorizeOverride} className="space-y-4">
                ...
                <div className="flex gap-2.5 pt-2">
                  <button
                    type="button"
                    onClick={() => { setShowOverrideModal(false); setPendingRefundTxId(null); }}
