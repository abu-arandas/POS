...
export default function CatalogPush({ orgId }: CatalogPushProps) {
  ...
  const runPreview = async () => {
    ...
    try {
      ...
      // ⚡ Bolt Optimization:
      // Replaced sequential for...of loop with Promise.all to fetch store data and plan concurrently.
      // This eliminates the bottleneck of waiting for each store's requests sequentially.
  ...
  const runPush = async () => {
    ...
    try {
      ...
      // ⚡ Bolt Optimization:
      // Replaced sequential for...of loop with Promise.all to fetch and push store catalog concurrently.
      // This eliminates the bottleneck of waiting for each store's requests sequentially.
