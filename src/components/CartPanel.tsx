  ...
  return (
    ...
    >
      <div
        ...
        className="p-4 border-b border-white/20 dark:border-white/10 bg-white/40 dark:bg-slate-900/40 flex items-center justify-between backdrop-blur-md"
      >
        <div className="flex items-center space-x-3 overflow-hidden">
          <div
            className={`p-2 rounded-xl shrink-0 transition-colors ${activeCustomer ? 'bg-emerald-500 shadow-lg shadow-emerald-500/20 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}
          >
            <User size={18} />
          </div>
      ...
      <div
        id="cart-promos-box"
        className="p-4 border-t border-white/20 dark:border-white/10 bg-white/30 dark:bg-slate-900/30 space-y-2.5 backdrop-blur-md"
      >
        {activeCustomer && activeCustomer.points > 0 && discountType !== 'loyalty' && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-2.5 flex items-center justify-between shadow-inner"
          >
        ...
        {discountType !== 'none' && !showPromoInput && (
          <motion.div
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
            className="flex items-center justify-between bg-amber-500/10 border border-amber-500/20 rounded-xl p-2 px-3 text-xs"
          >
          ...
          <div className="flex items-center gap-2">
            {!showPromoInput ? (
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="w-full flex items-center space-x-2 bg-white dark:bg-slate-900 border border-emerald-500/30 rounded-xl p-1 shadow-inner"
              >
                <input
                  type="number"
                  min="0"
      ...
      <div
        ...
        className="p-5 border-t border-white/20 dark:border-white/10 bg-white/50 dark:bg-slate-900/50 space-y-4 backdrop-blur-md"
      >
